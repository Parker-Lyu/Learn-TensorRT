# 30 - C++ Deployment Interview Katas
...
```bash
cmake -S 30_cpp_interview_katas -B 30_cpp_interview_katas/build
cmake --build 30_cpp_interview_katas/build --parallel
```
...
```bash
./30_cpp_interview_katas/build/katas_demo
```
...
```bash
ctest --test-dir 30_cpp_interview_katas/build --output-on-failure
```

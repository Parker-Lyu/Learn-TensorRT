    ...
    try {
        ...
        for (int index = 1; index < argc; ++index) {
            ...
            if (argument == "--input") inputs.push_back(value());
                ...
                if (policy == "round-robin") config.scheduling = lesson19::SchedulingPolicy::RoundRobin;
                else if (policy == "latest") config.scheduling = lesson19::SchedulingPolicy::LatestFirst;
        ...
        std::vector<lesson19::StreamConfig> streams;
        ...
        lesson19::MultiStreamPipeline pipeline(config, std::move(streams));

...
./31_final_portfolio_case_study/build_delivery_engine.sh
```

Build the local checkpoint matrix (generated `build/` directories are ignored):

```bash
./31_final_portfolio_case_study/build_local_checks.sh
```
...
```bash
python3 31_final_portfolio_case_study/run_local_checks.py
python3 31_final_portfolio_case_study/generate_case_study.py
```

The report is written to `reports/31_final_portfolio_case_study.md`; raw local-check output is
written to `31_final_portfolio_case_study/outputs/local_checks.json`. The JSON records each command,
...
DEVELOPMENT_IMAGE=learn-tensorrt:25.11 \
  ./31_final_portfolio_case_study/measure_images.sh
```
...
manifest under `31_final_portfolio_case_study/outputs/`. Run the image on an NVIDIA host:

```bash
docker run --rm --gpus all -v "$PWD/31_final_portfolio_case_study/outputs:/outputs" \
...
- `reports/31_final_portfolio_case_study.md` is ignored and regenerated from the current evidence.
...
```bash
python3 -m unittest discover -s 31_final_portfolio_case_study/tests -v
```

...
- `trt_inference_py` pybind11 extension and `python/trt_pybind.py` client
...
python3 29_cpp_shared_library_python_binding/python/trt_pybind.py --batch 2
```
...
the inference core. This lesson implements both paths: `ctypes` demonstrates stable ABI design,
while pybind11 demonstrates NumPy conversion, Python exceptions, RAII lifetime, and releasing the
GIL around the synchronous TensorRT call.
...
- `build/trt_inference_py*.so` is an ignored, environment-specific Python extension.
...
4. Compare C ABI and pybind11 trade-offs, including NumPy conversion and GIL behavior.

...
| CUDA kernel diagnosis and optimization with Nsight Compute | Covered as advanced path; Systems selects a source-owned LayerNorm before Compute explains fusion, with separate operator/network/deployment decisions | `20_cuda_preprocess_npp`, `31_nsight_compute_kernel_analysis` |

...
struct PipelineResult {
    std::vector<lesson10::Detection> detections;
...
PipelineResult run_pipeline(const cv::Mat& image,
                            lesson10::TensorRtRunner& runner,
                            const lesson10::PostprocessConfig& post_config,
                            const std::string& range_name) {
    lesson10::NvtxRange iteration_range(range_name);
    const auto total_start = std::chrono::steady_clock::now();

    const auto preprocess_start = std::chrono::steady_clock::now();
    lesson10::PreprocessResult preprocessed;
    {
        lesson10::NvtxRange range("preprocess");
        preprocessed = lesson10::preprocess_image(image, input_size_from_shape(runner.input_shape()));
    }
    const auto preprocess_stop = std::chrono::steady_clock::now();

    lesson10::InferenceOutput inference;
    {
        lesson10::NvtxRange range("inference");
    ...
    std::vector<lesson10::Detection> detections;
    {
        lesson10::NvtxRange range("postprocess");
        detections = lesson10::decode_yolov8_output(
...
void write_json(const std::filesystem::path& path,
    ...
    for (std::size_t index = 0; index < detections.size(); ++index) {
        const lesson10::Detection& detection = detections[index];
...
int main(int argc, char* argv[]) {
    try {
        ...
        const cv::Mat annotated = lesson10::draw_detections(image, final_result.detections);

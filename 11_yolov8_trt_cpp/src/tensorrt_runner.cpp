...
std::vector<int64_t> dims_to_vector(const nvinfer1::Dims& dims) {
    ...
    for (int i = 0; i < dims.nbDims; ++i) {
        if (dims.d[i] <= 0) {
            throw std::runtime_error("Only fully static tensor shapes are supported in lesson 11.");
        }
...
struct TensorRtRunner::Impl {
    explicit Impl(const std::string& path) {
        ...
        for (int i = 0; i < engine->getNbIOTensors(); ++i) {
            ...
            if (engine->getTensorLocation(tensor_name) != nvinfer1::TensorLocation::kDEVICE ||
                ...
                throw std::runtime_error(
                    "Lesson 11 supports only linear device IO tensors: " +
                    std::string(tensor_name));
            }
            if (engine->getTensorDataType(tensor_name) != nvinfer1::DataType::kFLOAT) {
                throw std::runtime_error("Lesson 11 expects float32 IO tensor: " +
                                         std::string(tensor_name));
            }
        ...
        if (input_count != 1 || output_count != 1) {
            throw std::runtime_error("Lesson 11 expects exactly one input and one output tensor.");
        }

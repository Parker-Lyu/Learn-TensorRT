...
int main(int argc, char** argv) {
    try {
        ...
        std::unique_ptr<lesson18::FrameSource> source;
        if (input.empty()) source = std::make_unique<lesson18::SyntheticSource>(synthetic_frames);
        else source = std::make_unique<lesson18::VideoSource>(input);
        lesson18::AsyncVideoPipeline pipeline(config, std::move(source));

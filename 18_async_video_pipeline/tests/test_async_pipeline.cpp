...
int main() {
    try {
        lesson18::PipelineConfig config;
        config.queue_capacity = 3;
        config.max_batch_size = 4;
        config.simulated_inference = std::chrono::milliseconds(2);
        lesson18::AsyncVideoPipeline normal(config,
            std::make_unique<lesson18::SyntheticSource>(100, cv::Size(32, 24)));
        ...
        bool failed = false;
        try {
            lesson18::AsyncVideoPipeline broken(config,
                std::make_unique<lesson18::SyntheticSource>(20, cv::Size(32, 24)));
            (void)broken.run();
        ...
        lesson18::AsyncVideoPipeline stopped(config,
            std::make_unique<lesson18::SyntheticSource>(10000, cv::Size(32, 24)));

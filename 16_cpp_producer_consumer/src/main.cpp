...
struct Options {
    lesson16::PipelineConfig pipeline;
...
lesson16::OverloadPolicy parse_policy(const std::string& value) {
    if (value == "block") {
        return lesson16::OverloadPolicy::Block;
    }
    if (value == "drop-newest") {
        return lesson16::OverloadPolicy::DropNewest;
    }
    if (value == "drop-oldest") {
        return lesson16::OverloadPolicy::DropOldest;
    }
...
const char* policy_name(lesson16::OverloadPolicy policy) {
    switch (policy) {
        case lesson16::OverloadPolicy::Block:
            return "block";
        case lesson16::OverloadPolicy::DropNewest:
            return "drop-newest";
        case lesson16::OverloadPolicy::DropOldest:
            return "drop-oldest";
    }
    return "unknown";
}
...
int main(int argc, char** argv) {
    try {
        ...
        lesson16::ImagePipeline pipeline(options.pipeline);
        const lesson16::PipelineStats stats = pipeline.run(options.images);

...
void test_invalid_capacity() {
    require_throws([] { lesson16::BoundedQueue<int> queue(0); }, "capacity");
}

void test_fifo_and_drain_close() {
    lesson16::BoundedQueue<int> queue(2);
    require(queue.push(1) == lesson16::PushResult::Pushed, "first push failed");
    require(queue.push(2) == lesson16::PushResult::Pushed, "second push failed");
    queue.close(lesson16::CloseMode::Drain);
    require(queue.push(3) == lesson16::PushResult::Closed, "closed queue accepted a push");
...
void test_drop_policies() {
    lesson16::BoundedQueue<int> newest(1, lesson16::OverloadPolicy::DropNewest);
    require(newest.push(1) == lesson16::PushResult::Pushed, "initial newest push failed");
    require(newest.push(2) == lesson16::PushResult::DroppedNewest,
            "drop-newest result mismatch");
    newest.close();
    require(newest.pop() == 1, "drop-newest kept the wrong value");

    lesson16::BoundedQueue<int> oldest(1, lesson16::OverloadPolicy::DropOldest);
    require(oldest.push(1) == lesson16::PushResult::Pushed, "initial oldest push failed");
    require(oldest.push(2) == lesson16::PushResult::DroppedOldest,
...
void test_close_wakes_blocked_threads() {
    ...
    lesson16::BoundedQueue<int> full_queue(1, lesson16::OverloadPolicy::Block);
    full_queue.push(1);
    ...
    full_queue.close(lesson16::CloseMode::Discard);
    require(producer.wait_for(1s) == std::future_status::ready,
            "close did not wake blocked producer");
    require(producer.get() == lesson16::PushResult::Closed,
    ...
    lesson16::BoundedQueue<int> empty_queue(1);
...
std::filesystem::path make_fixture() {
    const auto path = std::filesystem::temp_directory_path() /
                      ("lesson16_fixture_" + std::to_string(
...
void test_pipeline_overload_and_repeated_start_stop() {
    ...
    try {
        for (int iteration = 0; iteration < 20; ++iteration) {
            ...
            config.overload_policy = lesson16::OverloadPolicy::DropOldest;
            config.producer_delay = std::chrono::milliseconds(0);
            config.consumer_delay = std::chrono::milliseconds(1);
            lesson16::ImagePipeline pipeline(config);
...
void test_pipeline_failure_propagation() {
    ...
    try {
        ...
        config.overload_policy = lesson16::OverloadPolicy::Block;
        ...
        lesson16::ImagePipeline producer_failure_pipeline(config);

...
// TODO 1: Resize with unchanged aspect ratio and pad to input_size.
cv::Mat letterbox_image(const cv::Mat& bgr_image, cv::Size input_size, LetterboxInfo& info);

// TODO 3: Convert a batch of BGR HWC images into one RGB float32 NCHW tensor.
BatchPreprocessResult preprocess_batch_to_nchw(const std::vector<cv::Mat>& bgr_images,
                                               cv::Size input_size);

// TODO 4: Map a model box from letterboxed network coordinates back to original image coordinates.

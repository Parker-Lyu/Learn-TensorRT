# 24 - ONNX Graph Surgery and Plugin Escalation
...
deserialization must find the registered creator and compatible plugin library. Lesson 25 provides
...
PIP_INDEX_URL=https://pypi.tuna.tsinghua.edu.cn/simple \
  ./24_onnx_graph_surgery_plugin/setup_local_deps.sh
```
...
export PYTHONPATH=24_onnx_graph_surgery_plugin/.deps

python3 24_onnx_graph_surgery_plugin/create_demo_model.py
python3 24_onnx_graph_surgery_plugin/diagnose_model.py

trtexec --stronglyTyped --onnx=24_onnx_graph_surgery_plugin/outputs/unsupported_swish.onnx
# Expected failure: no importer for com.acme::AcmeSwish.

python3 24_onnx_graph_surgery_plugin/rewrite_with_graphsurgeon.py
python3 24_onnx_graph_surgery_plugin/validate_rewrite.py

trtexec \
  --stronglyTyped \
  --onnx=24_onnx_graph_surgery_plugin/outputs/rewritten_swish.onnx \
  --saveEngine=24_onnx_graph_surgery_plugin/outputs/rewritten_swish.engine
```
...
```bash
PYTHONNOUSERSITE=1 PYTHONPATH=24_onnx_graph_surgery_plugin/.deps \
python3 -m unittest discover -s 24_onnx_graph_surgery_plugin/tests -v
```

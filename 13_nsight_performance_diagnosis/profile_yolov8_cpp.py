#!/usr/bin/env python3
"""Measure steady-state latency and capture an Nsight Systems trace for lesson 11."""
...
def build_lesson11(lesson11_dir: Path) -> None:
    run_command(["cmake", "-S", ".", "-B", "build"], cwd=lesson11_dir)
    run_command(["cmake", "--build", "build", "-j2"], cwd=lesson11_dir)
...
def run_baseline(
    ...
    if not report_path.is_file():
        raise FileNotFoundError(f"lesson 11 did not create its JSON report: {report_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lesson11-dir", type=Path, default=Path("../11_yolov8_trt_cpp"))
...
def main() -> int:
    ...
    if not args.skip_build:
        build_lesson11(args.lesson11_dir)

    executable = args.lesson11_dir / "build" / "yolov8_trt_cpp"
    if not executable.is_file():
        raise FileNotFoundError(f"lesson 11 executable not found: {executable}")

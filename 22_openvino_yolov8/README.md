...
Lesson 22 uses an isolated layout:
...
```bash
./22_openvino_yolov8/setup_local_deps.sh
```
...
```bash
PYTHONNOUSERSITE=1 PYTHONPATH=22_openvino_yolov8/.deps python3 -c \
...
`22_openvino_yolov8/.deps/`. Use this lesson-local directory for every command in this lesson so
the benchmark cannot silently import a different user or system package.

Clean the lesson environment with:

```bash
rm -rf 22_openvino_yolov8/.deps 22_openvino_yolov8/outputs
```
...
```bash
PYTHONNOUSERSITE=1 PYTHONPATH=22_openvino_yolov8/.deps \
  python3 22_openvino_yolov8/run_openvino.py
PYTHONNOUSERSITE=1 PYTHONPATH=22_openvino_yolov8/.deps \
  python3 22_openvino_yolov8/generate_comparison.py
```
...
```bash
PYTHONNOUSERSITE=1 PYTHONPATH=22_openvino_yolov8/.deps \
22_openvino_yolov8/.deps/bin/benchmark_app \
...
```bash
PYTHONNOUSERSITE=1 PYTHONPATH=22_openvino_yolov8/.deps \
python3 -m unittest discover -s 22_openvino_yolov8/tests -v
```

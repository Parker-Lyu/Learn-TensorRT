# 25 - Custom TensorRT ScaleShift Plugin
...
```bash
cmake -S 25_custom_tensorrt_plugin -B 25_custom_tensorrt_plugin/build \
  -DCMAKE_BUILD_TYPE=Release
cmake --build 25_custom_tensorrt_plugin/build --parallel
```
...
```bash
python3 25_custom_tensorrt_plugin/create_plugin_model.py
/opt/tensorrt/bin/trtexec \
  --stronglyTyped \
  --staticPlugins=25_custom_tensorrt_plugin/build/libscale_shift_plugin.so \
  --onnx=25_custom_tensorrt_plugin/outputs/scale_shift.onnx \
  --saveEngine=25_custom_tensorrt_plugin/outputs/scale_shift.engine \
  --skipInference
./25_custom_tensorrt_plugin/build/validate_plugin \
  25_custom_tensorrt_plugin/outputs/scale_shift.engine
```

The convenience wrapper `25_custom_tensorrt_plugin/build_and_validate.sh` performs the same build

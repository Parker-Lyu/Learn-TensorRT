# 26 - DeepStream and GStreamer Multi-Stream Analytics
...
```bash
./26_deepstream_gstreamer_multistream/build_runtime_assets.sh
```

## Run

```bash
python3 26_deepstream_gstreamer_multistream/generate_app_config.py \
  --source /data/camera-a.mp4 \
  --source /data/camera-b.mp4

cd 26_deepstream_gstreamer_multistream/outputs
...
```bash
python3 -m unittest discover -s 26_deepstream_gstreamer_multistream/tests -v
python3 26_deepstream_gstreamer_multistream/validate_config.py
```
...
```bash
mkdir -p 26_deepstream_gstreamer_multistream/outputs
nvidia-smi --query-gpu=name,compute_cap,driver_version,memory.total \
  --format=csv > 26_deepstream_gstreamer_multistream/outputs/gpu_platform.csv
deepstream-app --version-all \
  > 26_deepstream_gstreamer_multistream/outputs/deepstream_versions.txt
nvidia-smi dmon -s u -d 1 \
  > 26_deepstream_gstreamer_multistream/outputs/gpu_utilization.log
```

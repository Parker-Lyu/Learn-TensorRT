...
DEPS_DIR=${DEPS_DIR:-"${ROOT_DIR}/17_dynamic_batching/.deps"}
...
if [[ ! -d "${DEPS_DIR}/onnx_graphsurgeon" ]]; then
  echo "AutoCast dependencies are missing. Run 17_dynamic_batching/setup_autocast_deps.sh" >&2
  exit 2
fi

mkdir -p "${SCRIPT_DIR}/outputs" "$(dirname -- "${TIMING_CACHE}")"
PYTHONPATH="${DEPS_DIR}${PYTHONPATH:+:${PYTHONPATH}}" \
  python3 "${ROOT_DIR}/17_dynamic_batching/prepare_autocast_input.py" \

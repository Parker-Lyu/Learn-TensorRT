#!/usr/bin/env python3
"""Render lesson 10a's report from saved validation artifacts."""
...
OUTPUT_DIR = REPO_ROOT / "10a_end_to_end_validation_report" / "outputs"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the lesson 10a report from saved JSON and log artifacts."
    )
    parser.add_argument(
        "--environment-log",
        type=Path,
    ...
    parser.add_argument(
        "--precision-report",
        type=Path,
        default=REPO_ROOT / "06a_polygraphy_precision_alignment" / "outputs" / "precision_report.json",
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPO_ROOT / "reports" / "10a_end_to_end_validation.md",
    )
...
def validate(
    ...
    if onnx != required_path(precision, "onnx", "precision report"):
        raise ValueError("ONNX artifact mismatch between lesson 05 and lesson 06a")

    engine = required_path(precision, "engine", "precision report")
    if engine != required_path(cpp, "engine", "C++ report"):
        raise ValueError("TensorRT engine mismatch between lesson 06a and lesson 10")
    ...
    if required_path(precision, "input_npy", "precision report") != expected_input:
        raise ValueError("lesson 06a did not use lesson 05's saved NCHW tensor")
    ...
    if not isinstance(trt_comparison, dict) or not isinstance(trt_comparison.get("metrics"), dict):
        raise ValueError("TensorRT alignment is missing; re-run lesson 06a without --skip-trt")
...
def render(evidence: dict[str, Any], environment_log: Path) -> str:
    ...
    return f"""# 10a - End-to-End Validation Report

...
struct Box { float x1, y1, x2, y2, score; int class_id; };
...
struct Image { int width, height, channels; std::vector<float> data_hwc; };
float bilinear_sample(const Image& image, float x, float y, int channel);
std::vector<float> hwc_to_chw(const Image& image);

struct Letterbox { float scale; float pad_x; float pad_y; int original_width; int original_height; };
...
public:
    explicit RingBuffer(std::size_t capacity) : values_(capacity) {
        if (!capacity) throw std::invalid_argument("ring capacity must be positive");
    }
    bool push(T value) {
        if (size_ == values_.size()) return false;
        values_[(head_ + size_) % values_.size()] = std::move(value); ++size_; return true;
    }
    std::optional<T> pop() {
        if (!size_) return std::nullopt;
        T value = std::move(values_[head_]); head_ = (head_ + 1) % values_.size(); --size_; return value;
    }
    std::size_t size() const { return size_; }
private:
    std::vector<T> values_; std::size_t head_{0}; std::size_t size_{0};
};
...
public:
    ...
    bool push(T value) {
        ...
        values_.push_back(std::move(value)); readable_.notify_one(); return true;
    }
    std::optional<T> pop() {
        std::unique_lock<std::mutex> lock(mutex_);
        readable_.wait(lock, [&] { return closed_ || !values_.empty(); });
        if (values_.empty()) return std::nullopt;
        T value = std::move(values_.front()); values_.pop_front(); writable_.notify_one(); return value;
    }
    void close() {
...
private:
    std::size_t capacity_; std::mutex mutex_; std::condition_variable readable_, writable_;
    std::deque<T> values_; bool closed_{false};
};

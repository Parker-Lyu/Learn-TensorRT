...
const char* name(lesson17::HostMemoryMode mode) {
    if (mode == lesson17::HostMemoryMode::Pageable) return "pageable";
    if (mode == lesson17::HostMemoryMode::Pinned) return "pinned";
    return "mapped";
}
}
...
int main(int argc, char** argv) {
    try {
        ...
        for (int iteration = 0; iteration < iterations; ++iteration) {
            ...
            reference = lesson17::cpu_resize_bgr_to_rgb_nchw(image, target);
        ...
        for (const auto mode : {lesson17::HostMemoryMode::Pageable,
            ...
            lesson17::GpuPreprocessor gpu(image.size(), target, mode);
            (void)gpu.run(image);
            std::vector<double> stage, h2d, preprocess, d2h;
            lesson17::GpuPreprocessResult result;
            for (int iteration = 0; iteration < iterations; ++iteration) {

# 21 - Pipeline Performance and Reliability Report

## Purpose

This checkpoint converts lessons 16–20 into reproducible load, latency, fairness, memory, fault,
and sanitizer evidence. It generates `reports/21_pipeline_performance.md`; it does not silently
mark a short smoke run as a completed 30-minute soak.

Build lessons 16, 18, 19, and 20 first, then run from the repository root:

## Prerequisites

- Build lessons 16, 18, 19, and 20 first.
...
```bash
python3 21_pipeline_performance_report/collect_pipeline_evidence.py
python3 21_pipeline_performance_report/generate_report.py
```
...
- The generated `reports/21_pipeline_performance.md` is ignored and must state incomplete formal gates explicitly.
...
```bash
python3 -m unittest discover -s 21_pipeline_performance_report/tests -v
```

## Checkpoints

1. Generate reproducible load, latency, throughput, fairness, memory, and stability evidence for lessons 16 through 20.

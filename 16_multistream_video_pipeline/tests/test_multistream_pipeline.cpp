...
int main() {
    try {
        ...
        const std::vector<lesson16::StreamConfig> streams = {
            {7, "", 60, std::chrono::milliseconds(0)},
            {42, "", 60, std::chrono::milliseconds(1)}};
        lesson16::MultiStreamPipeline pipeline(config, streams);
        ...
        lesson16::MultiStreamPipeline isolated(config, isolated_streams);
        ...
        bool failed = false;
        try { lesson16::MultiStreamPipeline broken(config, streams); (void)broken.run(); }

...
int main(int argc, char** argv) {
    try {
        ...
        lesson17::DynamicBatchRunner runner(options.engine.string());
        ...
        for (const std::size_t batch : {1U, 2U, 4U}) {
            const auto input = lesson17::make_batched_input(batch, 3, 640, 640);
            for (int iteration = 0; iteration < options.warmup; ++iteration) runner.infer(input, batch);
            std::vector<lesson17::InferenceTiming> samples;

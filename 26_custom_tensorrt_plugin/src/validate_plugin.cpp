...
std::vector<char> read_file(const std::string& path) {
    ...
    input.seekg(0);
    if (!input.read(bytes.data(), static_cast<std::streamsize>(bytes.size()))) throw std::runtime_error("failed while reading engine: " + path);
    return bytes;
}
class DeviceBuffer {
public:
...
int main(int argc, char** argv) {
    try {
        ...
        if (!initAcmeSwishPlugin()) throw std::runtime_error("plugin library initialization failed");
        Logger logger;
        ...
        if (!context || engine->getNbIOTensors() != 2) throw std::runtime_error("expected one input and one output tensor");
        ...
        Stream stream;
        if (!context->setTensorAddress(input_name, device_input.get()) || !context->setTensorAddress(output_name, device_output.get())) {
            throw std::runtime_error("failed to set TensorRT tensor addresses");
        }
        check_cuda(cudaMemcpyAsync(device_input.get(), input.data(), input.size() * sizeof(float), cudaMemcpyHostToDevice, stream.get()), "copy input");
        if (!context->enqueueV3(stream.get())) throw std::runtime_error("enqueueV3 failed");
        check_cuda(cudaMemcpyAsync(output.data(), device_output.get(), output.size() * sizeof(float), cudaMemcpyDeviceToHost, stream.get()), "copy output");
        check_cuda(cudaStreamSynchronize(stream.get()), "wait for output");
        float max_error = 0.0F;
        for (std::size_t index = 0; index < input.size(); ++index) {
            const float biased = input[index] + 0.25F;
            const float reference = biased / (1.0F + std::exp(-biased)) * 1.5F;
            max_error = std::max(max_error, std::abs(output[index] - reference));
        }
        if (max_error > 1e-5F) throw std::runtime_error("full graph numerical validation failed");
        std::cout << "AcmeSwish IPluginV3 full_graph_max_abs=" << max_error << '\n';
        return EXIT_SUCCESS;
